...
import { addToCartSchema, updateCartSchema } from "../../validations/cart.schema";
...
const formatCart = (cart: Awaited<ReturnType<typeof getOrCreateCart>>) => {
  ...
  // Distinct product count — e.g. 1 product with quantity 5 should still
  // show "1" on the navbar cart badge, not "5". itemCount (total quantity)
  // is kept as-is since the cart page's "X items" text relies on it.
